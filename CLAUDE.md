# Using KnowledgeBook with an AI coding assistant
...
`knowledgebook/ai/<name>_backend.py` modeled on `claude_backend.py`,
add config keys to `knowledgebook/config.py`, add the kind to
...
1. Create `knowledgebook/skills/<name>_skill.py` subclassing `Skill`.
   Implement `async def execute(self, params: dict) -> SkillResult`.
2. Register in `knowledgebook/orchestrator/engine.py` (`self.skills`).

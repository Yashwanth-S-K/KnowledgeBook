"""Merge the latest 14-day traffic window into the cumulative clone history.

Vendored from https://github.com/MShawon/github-clone-count-badge (main.py) so the
workflow does not curl a script from a third-party repo at run time.

Reads:  clone.json (latest 14-day window from the traffic API)
        clone_before.json (cumulative history stored in the gist)
Writes: clone.json (merged cumulative history + total count/uniques)
"""
...
# Once the history grows past ~100 daily entries, roll the oldest ones up per month
# so the gist payload stays small.
